...
REAL_STALL = """\
2026-09-07T17:06:59.6028585Z Error when talking to Gemini API Full report available at: /tmp/x.json TypeError: terminated
...
def _classify(log_text: str) -> bool:
    """Run the workflow's own classifier over a log, returning whether it
    would re-run. Extracted from the step so the test cannot drift from it."""
    sed = re.search(r"TAIL=\$\((sed -E [^)]+)\| tail -n 40\)", STEP)
    assert sed, "the classifier's tail extraction could not be located"
    fatal = re.search(r"FATAL='([^']+)'", STEP).group(1)
    transport = re.search(r"TRANSPORT='([^']+)'", STEP).group(1)
    script = f"""
set -euo pipefail
cat > failed.log
TAIL=$({sed.group(1)}failed.log | tail -n 40)
FATAL='{fatal}'
TRANSPORT='{transport}'
if printf '%s\\n' "$TAIL" | grep -qE "$FATAL" \\
   && printf '%s\\n' "$TAIL" | grep -qE "$TRANSPORT"; then
  echo RERUN
else
  echo LEAVE_RED
fi
