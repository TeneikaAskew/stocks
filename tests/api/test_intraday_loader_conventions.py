...
def _session(day: str, *, stored: str, start="04:00", end="19:59") -> pd.DataFrame:
...
def _expected(day: str, start="04:00", end="19:59") -> pd.DatetimeIndex:
...
@pytest.mark.parametrize("day,stored", [
    ...
    ("2026-01-15", "utc"),        # new writers, EST: 19:00-19:59 ET spills past UTC midnight
])
def test_each_convention_reads_as_the_same_eastern_session(day, stored):
    df = _session(day, stored=stored)
    idx = main_module._intraday_index_to_eastern(df["ts"], df["volume"])
...
def test_chart_endpoint_opens_at_0930_for_both_conventions(client, monkeypatch, stored):
    ...
    assert seen["end"] == pd.Timestamp("2026-09-25 01:00", tz="UTC")
