...
This is the GCP-runnable face of the backend: dispatched as the magnitude-engine
Cloud Run Job (research image) it reads market_data_daily over the Cloud SQL
connector and emits to Cloud Logging.
...
def _fwd_outcome(history: list, i: int) -> str:
    """Causal forward-walk: what the NEXT completed bar did vs this bar's range."""
    if i + 1 >= len(history):
...
def report_ticker(ticker: str, days: int) -> None:
    ...
    # ── Daily forward-walk over the past week ──────────────────────────────
    hist = d["history"]
    ...
    for i in range(start, len(hist)):
        ...
        print(f"    {b['period']:<11}{b['candle']:<5}{(b['combo'] or 'none'):<26}"
              ...
              f"   {trig:<18}{outcome}")
        # directional setup hit-rate: did a directional bar's continuation follow?
    ...
    # ── Higher timeframes: current classification ──────────────────────────
...
def main():
    ...
    p.add_argument("--days", type=int, default=7, help="daily sessions to show")
    args = p.parse_args()
