#!/usr/bin/env python3
"""
Prepare data for GitHub Pages deployment by converting AlphaVantage parquet data
to daily JSON files. Processes both monthly and daily parquet formats.
"""
...
def prepare_github_pages_data():
    """Convert AlphaVantage parquet data to daily JSON files for GitHub Pages."""
            ...
            if data_format == "alphavantage_monthly":
                ...
                for daily_file in files_to_process:
                    ...
                    if df.index.size > 0:
                        ...
                        date_str = date.strftime('%Y%m%d')
                        all_dates.append(date_str)
                        create_daily_json(df, ticker_dest, date_str)
    ...
    print(f"\nNote: Recent data deployed to GitHub Pages.")
