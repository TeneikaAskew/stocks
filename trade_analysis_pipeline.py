...
1. Read {ticker}_trade_tracker.csv from data/signals/trade_examples/
2. Calculate durations and update to {ticker}_trade_tracker_updated.csv
...
class TradeAnalysisPipeline:
    ...
    def _sig_path(self, filename):
        """Return data/signals/{ticker}_{filename} path for this pipeline run."""
        return f"data/signals/{self.symbol.lower()}_{filename}"
    ...
    def step1_update_trade_tracker(self):
        ...
        # Read ticker-specific trade tracker
        tracker_path = f'data/signals/trade_examples/{self.symbol.lower()}_trade_tracker.csv'
        ...
        print(f"Loaded {len(self.trades_df)} trades from {tracker_path}")
    ...
    def step3_join_indicators(self):
        ...
        if self.data_format == 'parquet':
            enriched_final.to_parquet(self._sig_path('trades_enriched.parquet'), index=False)
            print(f"Saved {self._sig_path('trades_enriched.parquet')} with entry/exit indicators")
        else:
            enriched_final.to_csv(self._sig_path('trades_enriched.csv'), index=False)
            print(f"Saved {self._sig_path('trades_enriched.csv')} with entry/exit indicators")
    ...
    def create_pattern_summary(self, patterns):
        ...
        # Write summary
        with open(self._sig_path('pattern_summary.md'), 'w', encoding='utf-8') as f:
            f.write('\n'.join(summary_lines))

        print(f"Created pattern summary: {self._sig_path('pattern_summary.md')}")
    ...
    def step5_find_similar_trades(self, patterns):
        ...
        for i in range(1, len(search_df)-max_duration):
            ...
            # Skip entries after 3:55 PM (need room for exit by 3:58 PM)
            current_time = current['Time']
            if current_time.hour > 15 or (current_time.hour == 15 and current_time.minute > 55):
                continue
            ...
            if (current['Price_Change_1m'] > 0.01 and 
                ...
                for duration in range(4, 32):  # Check every minute from 4 to 31
                    if i + duration < len(search_df):
                        ...
                        # Ensure exit is same day and by 3:58 PM
                        if exit_time.date() != current_time.date():
                            break
                        if exit_time.hour > 15 or (exit_time.hour == 15 and exit_time.minute > 58):
                            break
    ...
    def step6_criteria_analysis(self, enriched_df, output_filename=None, is_similar=False):
        ...
        if output_filename is None:
            output_filename = self._sig_path('trade_criteria_analysis.csv')
    ...
    def step7_criteria_insights(self):
        ...
        criteria_df = self._get_cached_df('similar_trades', self._sig_path('similar_trades_pipeline.csv'))
        ...
        os.makedirs('data/signals', exist_ok=True)
        if self.data_format == 'parquet':
            criteria_results_df.to_parquet(self._sig_path('criteria_effectiveness.parquet'), index=False)
            print(f"\nSaved detailed criteria effectiveness to: {self._sig_path('criteria_effectiveness.parquet')}")
        else:
            criteria_results_df.to_parquet(self._sig_path('criteria_effectiveness.csv'), index=False)
            print(f"\nSaved detailed criteria effectiveness to: {self._sig_path('criteria_effectiveness.csv')}")
    ...
    def update_criteria_summary_report(self, criteria_results_df, criteria_df):
        ...
        summary_file = self._sig_path('trade_criteria_summary.md')
        existing_content = []
    ...
    def _render_comprehensive_section(self, analysis):
        """Return a list of markdown lines for one comprehensive analysis window."""
        lines = []
        if 'executive_summary' in analysis:
            lines.append("#### Executive Summary")
            lines.extend(analysis['executive_summary'])
            lines.append("")

        if 'discriminating_indicators' in analysis:
            lines.append("#### Most Discriminating Indicators")
            lines.append("These indicators show the biggest difference between CALL and PUT trades:\n")
            lines.append("| Indicator | CALL % | PUT % | Difference |")
            lines.append("|-----------|--------|-------|------------|")
            for ind in analysis['discriminating_indicators'][:5]:
                lines.append(f"| {ind['name']} | {ind['call_pct']}% | {ind['put_pct']}% | {ind['difference']}% |")
            lines.append("")

        if 'powerful_combinations' in analysis:
            lines.append("#### Powerful Indicator Combinations")
            if 'call_combinations' in analysis['powerful_combinations']:
                lines.append("\n**CALL Combinations:**")
                for combo in analysis['powerful_combinations']['call_combinations']:
                    lines.append(f"- **{combo['name']}**: {combo['count']} trades ({combo['percentage']}%), Avg Return: {combo['avg_return']}%")
            if 'put_combinations' in analysis['powerful_combinations']:
                lines.append("\n**PUT Combinations:**")
                for combo in analysis['powerful_combinations']['put_combinations']:
                    lines.append(f"- **{combo['name']}**: {combo['count']} trades ({combo['percentage']}%), Avg Return: {combo['avg_return']}%")
            lines.append("")
        return lines
    ...
    def generate_analysis_report(self, patterns, comprehensive_results=None, enriched_df=None):
        ...
        report_lines = []
        report_lines.append(f"# Trade Analysis Report — {self.symbol}")
        report_lines.append(f"\nGenerated: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
        report_lines.append(f"\nTicker: **{self.symbol}**")
        report_lines.append("\n---\n")

        # Summary statistics
        date_min = pd.to_datetime(self.trades_df['Time'].min()).strftime('%Y-%m-%d')
        date_max = pd.to_datetime(self.trades_df['Time'].max()).strftime('%Y-%m-%d')
        report_lines.append("## Summary Statistics")
        report_lines.append(f"\n- **Ticker**: {self.symbol}")
        report_lines.append(f"- **Total Trades Analyzed**: {len(self.trades_df)}")
        ...
        enriched_df = self._get_cached_df('trades_enriched', self._sig_path('trades_enriched.csv'))
        ...
        if os.path.exists(self._sig_path('similar_trades_pipeline.csv')):
            similar_df = self._get_cached_df('similar_trades', self._sig_path('similar_trades_pipeline.csv'))
            if len(similar_df) > 0:
                report_lines.append("\n## Similar Trades Found\n")
        ...
        if os.path.exists(self._sig_path('criteria_effectiveness.csv')):
            criteria_results_df = self._get_cached_df('criteria_effectiveness', self._sig_path('criteria_effectiveness.csv'))
            criteria_df = self._get_cached_df('similar_trades', self._sig_path('similar_trades_pipeline.csv'))
        ...
        # Add comprehensive analysis — support dual-window comparison
        has_1m = hasattr(self, 'comprehensive_analysis_1m') and self.comprehensive_analysis_1m
        has_6m = hasattr(self, 'comprehensive_analysis_6m') and self.comprehensive_analysis_6m
        has_any = hasattr(self, 'comprehensive_analysis') and self.comprehensive_analysis

        if has_1m and has_6m:
            # --- Dual-window view ---
            report_lines.append("\n## Comprehensive Trading Analysis\n")
            report_lines.append("*Similar historical trades compared across two lookback windows.*\n")

            for label, analysis in [("1-Month Window", self.comprehensive_analysis_1m),
                                     ("6-Month Window", self.comprehensive_analysis_6m)]:
                report_lines.append(f"### {label}")
                report_lines.extend(self._render_comprehensive_section(analysis))
                report_lines.append("")

            # Comparison table
            a1 = self.comprehensive_analysis_1m
            a6 = self.comprehensive_analysis_6m
            es1 = {line.split(':', 1)[0].strip('- '): line.split(':', 1)[1].strip()
                   for line in a1.get('executive_summary', []) if ':' in line}
            es6 = {line.split(':', 1)[0].strip('- '): line.split(':', 1)[1].strip()
                   for line in a6.get('executive_summary', []) if ':' in line}
            report_lines.append("### Window Comparison")
            report_lines.append("| Metric | 1 Month | 6 Months |")
            report_lines.append("|--------|---------|----------|")
            for key in ['Total Similar Trades Analyzed', 'Date Range',
                        'CALL Win Rate', 'PUT Win Rate',
                        'Average CALL Return', 'Average PUT Return']:
                v1 = es1.get(key, '—')
                v6 = es6.get(key, '—')
                report_lines.append(f"| {key} | {v1} | {v6} |")
            report_lines.append("")

        elif has_any:
            # --- Single-window view (original behaviour) ---
            report_lines.append("\n## Comprehensive Trading Analysis\n")
            report_lines.append("*Based on analysis of similar historical trades*\n")
            report_lines.extend(self._render_comprehensive_section(self.comprehensive_analysis))
            report_lines.append("")

        # Criteria effectiveness (always from the primary window)
        if has_any:
            ...
            # Detailed analysis (key insights + time patterns) from primary results
        ...
        os.makedirs('data/signals', exist_ok=True)
        with open(self._sig_path('trade_analysis_report.md'), 'w', encoding='utf-8') as f:
            f.write('\n'.join(report_lines))

        print(f"\nGenerated unified trade analysis report: {self._sig_path('trade_analysis_report.md')}")
    ...
    def run_pipeline(self):
        ...
        if len(similar_df) > 0:
            ...
            print("="*60)
            self.step6_criteria_analysis(similar_df, self._sig_path('similar_trades_pipeline.csv'), is_similar=True)
        
        # Step 7: Analyze criteria effectiveness
        self.step7_criteria_insights()
        
        # Step 8: Comprehensive analysis of similar trades (primary window)
        ...
        if len(similar_df) > 0:
            ...
            self.comprehensive_analysis_1m = self.comprehensive_analysis

            # Run 6-month comparison window when the primary window is shorter
            compare_months = 6
            primary_months = self.search_months if self.search_months is not None else 0
            if primary_months > 0 and primary_months < compare_months:
                print(f"\nRunning {compare_months}-month comparison window for report...")
                original_months = self.search_months
                self.search_months = compare_months
                similar_df_6m = self.step5_find_similar_trades(patterns)
                self.search_months = original_months
                self._cache['similar_trades'] = None  # clear cache so step8 uses the new df
                if len(similar_df_6m) > 0:
                    results_6m = self.step8_comprehensive_analysis(df=similar_df_6m)
                    self.comprehensive_analysis_6m = self._prepare_comprehensive_summary(results_6m)
                self._cache['similar_trades'] = None  # restore clean state
        ...
        t = self.symbol.lower()
        print("\nOutput files:")
        print(f"  1. {self._sig_path('trades_enriched.csv')} - With entry/exit indicators")
        print(f"  2. {self._sig_path('trade_criteria_analysis.csv')} - Comprehensive criteria evaluation for actual trades")
        print(f"  3. {self._sig_path('similar_trades_pipeline.csv')} - Similar trades with full criteria analysis")
        print(f"  4. {self._sig_path('criteria_effectiveness.csv')} - Analysis of which criteria work best")
        print(f"  5. {self._sig_path('trade_analysis_report.md')} - Complete unified analysis report (includes all insights)")

    def step8_comprehensive_analysis(self, df=None):
        """Step 8: Comprehensive analysis of similar trades data.

        Parameters
        ----------
        df : DataFrame, optional
            Pre-loaded similar trades DataFrame. If None, loads from the
            cached similar_trades_pipeline file (normal pipeline flow).
        """
        print("\n" + "="*60)
        ...
        if df is None:
            df = self._get_cached_df('similar_trades', self._sig_path('similar_trades_pipeline.csv'))

        # Ensure Trade_Profitable exists — step5 output doesn't include it; step6 normally adds it.
        # When called with a raw step5 DataFrame (e.g. 6-month comparison), derive it here.
        if 'Trade_Profitable' not in df.columns:
            df = df.copy()
            df['Trade_Profitable'] = (df['Return_Pct'] > 0).astype(int)
        ...
        # 1. Basic Statistics
        print("\n1. BASIC STATISTICS:")
        sim_date_col = 'Entry_Time' if 'Entry_Time' in df.columns else df.columns[0]
        results['basic_stats'] = {
            'total_trades': len(df),
            ...
            'date_min': pd.to_datetime(df[sim_date_col].min()).strftime('%Y-%m-%d'),
            'date_max': pd.to_datetime(df[sim_date_col].max()).strftime('%Y-%m-%d'),
        }
    ...
    def _prepare_comprehensive_summary(self, results):
        ...
        summary['executive_summary'] = [
            ...
            f"- Date Range: {results['basic_stats']['date_min']} to {results['basic_stats']['date_max']}",
    ...
    def step9_generate_comprehensive_report(self, results):
        ...
        # Write report
        report_path = self._sig_path('comprehensive_analysis_report.md')
