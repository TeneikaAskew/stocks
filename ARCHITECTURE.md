...
**Job config drift (declared vs live)** (6): `compute-earnings-reactions.memory: repo `1Gi` live `2Gi``, `compute-earnings-reactions.max_retries: repo `1` live `0``, `db-query.memory: repo `512Mi` live `8Gi``, `db-query.cpu: repo `1` live `2``, `phase6-playbook.tasks: repo `3` live `1``, `strat-engine.memory: repo `8Gi` live `16Gi``
**Jobs whose last execution failed** (1): `intraday-bulk-backfill`
**Jobs that have never executed** (0): none
<!-- inventory:reconcile:end -->

Interpretation (2026-09-07): the live-only jobs are hand-created research jobs from May plus `exec-backtest` (research image `research-exec-backtest`, last run 2026-05-27); `p2-build-gamma-levels` is the one that matters because a scheduler depends on it (#829, #834). Schedulers reconcile exactly: `signal-quality-report-hourly` (retired by #1005) was deleted live on 2026-09-07. `gamma-levels-daily` still targets a job `deploy.sh` never creates. Every job has executed at least once: the latest execution is read per job from `status.latestCreatedExecution`, so a weekly or on-demand job is no longer hidden behind the five-minute options refresh; the two non-green latest runs are `intraday-bulk-backfill` (failed 2026-05-23, a one-off backfill) and `strat-dir-features` (cancelled 2026-05-27, hand-created research). Live table drift (26 runtime relations) is in §5.2. Six job fields differ between `deploy.sh` and the live job: `compute-earnings-reactions` runs at 2 GiB with no retry against a declared 1 GiB and one, `db-query` at 8 GiB / 2 CPU against 512 MiB / 1, `strat-engine` at 16 GiB against 8, and `phase6-playbook` at one task against three. Each was raised by hand after a failure; a `deploy.sh` rebuild would silently undo all six, so the declarations should be raised to match rather than the live values lowered.
