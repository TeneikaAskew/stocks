...
ALTER TABLE earnings_calendar
    ...
    -- evaluate_ew_strikes.py once the pick's scoring session (#1151) closes.
    ...
    -- Time-to-hit in minutes from the scoring session's first bar
    ...
    -- The session's directional bias: signed open-to-close %. Lets the
...
CREATE TABLE IF NOT EXISTS historical_signals (
    ...
    return_pct        DOUBLE PRECISION,             -- 20-min MFE. PERCENTAGE POINTS (0.5 = +0.5%), as is every return below; signal_metrics stores fractions (#1154)
...
-- ===========================================================================
-- Earnings frontend data prep — 3 materialized views / tables
-- ===========================================================================
...
-- earnings_calendar so the FastAPI router (platform/api/routers/earnings.py)
-- can serve in <10ms instead of running 5-table joins on every request.
--
-- These MUST come AFTER their source tables, all of which are defined
-- earlier in this file:
--   earnings_options_snapshots          (~line 404)
--   earnings_calendar                   (~line 505)
--   earnings_history                    (~line 666)
--   earnings_reactions                  (~line 712)
--   earnings_options_strategy_winners   (~line 3122)
--
-- Refreshed by gcp/refresh_earnings_views.py (Cloud Run Job
-- `refresh-earnings-views`):
--   - earnings_event_outcomes + earnings_ticker_lean: weekly Sun 20:00 ET
--   - earnings_upcoming_with_history: daily 07:30 ET (before the brief)
--
-- IDEMPOTENCY / RE-RUNNABILITY: schema.sql is applied repeatedly by the
-- `apply-schema-migrations` job (gcp/apply_schema.py). The two mat views use
-- DROP MATERIALIZED VIEW IF EXISTS … CASCADE then CREATE … WITH NO DATA so a
-- re-apply picks up any DDL change to the view definition (mat views cannot be
-- CREATE-OR-REPLACE'd when the column set changes). After a re-apply the views
-- are EMPTY until the next refresh job run repopulates them — this is by
-- design and acceptable (schema applies are deliberate; the refresh Job runs
-- weekly/daily). earnings_upcoming_with_history is a real TABLE refreshed
-- daily with live data, so it uses CREATE TABLE IF NOT EXISTS (NOT drop) to
-- preserve already-loaded rows across schema re-applies.
--
...
-- UNIQUE INDEX on (ticker, reported_date) → REFRESH … CONCURRENTLY-safe.
...
-- UNIQUE INDEX on (ticker) → REFRESH … CONCURRENTLY-safe.
...
-- refresh job via DELETE + INSERT. Created with CREATE TABLE IF NOT EXISTS
-- (NOT drop-and-recreate) so a schema re-apply preserves the rows the daily
-- job already loaded. Decorates the next N days of earnings_calendar
-- reporters with their lean stats so the frontend can render the
-- "this week" page in a single query.
