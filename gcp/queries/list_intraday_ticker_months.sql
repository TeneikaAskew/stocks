...
--
-- A month is keyed by (ts - 1 h) in UTC, the same framing as
-- fetch_alphavantage_intraday.month_replace_window: [M-01 01:00Z, M+1-01 01:00Z)
    ...
    SELECT m.ticker,
           (SELECT min(i.ts) FROM market_data_intraday i
             WHERE i.ticker = m.ticker AND i.interval = '1min'
               AND i.ts >= ((date_trunc('month', (m.ts AT TIME ZONE 'UTC') - interval '1 hour')
                             + interval '1 month' + interval '1 hour') AT TIME ZONE 'UTC'))
      FROM m WHERE m.ts IS NOT NULL
)
SELECT ticker, to_char((ts AT TIME ZONE 'UTC') - interval '1 hour', 'YYYY-MM') AS month
  FROM m
