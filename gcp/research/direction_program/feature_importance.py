...
def aggregate_importance(feature_cols, per_fold_gain, per_fold_shap):
    ...
    Raises RuntimeError (not a bare numpy crash) if a caller passes per-fold
    rows whose length doesn't match len(feature_cols) — e.g. a ragged
    per_fold_gain/per_fold_shap, or a shap row from an unexpected SHAP output
    shape. Production incident 2026-07-08 (issue #704, direction-importance):
    a length mismatch upstream made `mean_shap` end up multi-dimensional,
    which crashed inside `np.isnan(ms)` with "the truth value of an array
    with more than one element is ambiguous" — a correct symptom but useless
    for diagnosing which cell / which axis produced the bad shape. Validating
    here turns that into an actionable error naming the actual shapes.
    """
    ...
    try:
        ...
        raise RuntimeError(
            f"aggregate_importance: per_fold_gain rows have inconsistent "
            f"lengths {[len(r) for r in per_fold_gain]} (expected every row "
            f"to have {nfeat} entries, one per feature_col): {e}"
        ) from e
    if G.ndim != 2 or G.shape[1] != nfeat:
        raise RuntimeError(
            f"aggregate_importance: per_fold_gain has shape {G.shape}, "
            f"expected (n_folds, {nfeat}) to match {nfeat} feature_cols — "
            f"a per-fold gain row has the wrong length."
        )
    ...
    if shap_rows:
        try:
            ...
            raise RuntimeError(
                f"aggregate_importance: per_fold_shap rows have inconsistent "
                f"lengths {[len(r) for r in shap_rows]} (expected every row "
                f"to have {nfeat} entries, one per feature_col): {e}"
            ) from e
        if S.ndim != 2 or S.shape[1] != nfeat:
            raise RuntimeError(
                f"aggregate_importance: per_fold_shap has shape {S.shape}, "
                f"expected (n_folds_with_shap, {nfeat}) to match {nfeat} "
                f"feature_cols — a per-fold SHAP row has the wrong length."
            )
