...
| C-45 | `gcp/fetchers/evaluate_ew_strikes.py:232-235` | Same manual skip loop | I | Same as C-44 in EW path | Same fix | S | fixed with #1151: sessions come from the NYSE calendar |
| C-46 | `gcp/fetchers/evaluate_ew_strikes.py:190-192` | `bars_cache[key] = bars` no TTL | I | Multi-day-old intraday bars used; EW wave counts wrong | TTL by session-date; invalidate at 4:00 PM ET close | M | fixed with #1151: one fetch per (ticker, session) per run, no cross-day cache |
