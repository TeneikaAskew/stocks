...
**Method.** Every claim was checked against one of three sources and the source is named in each finding: the repo at `main` `259c223` plus the three open PRs that live GCP already reflects ([#990](https://github.com/TeneikaAskew/stocks/pull/990), [#1004](https://github.com/TeneikaAskew/stocks/pull/1004), [#1005](https://github.com/TeneikaAskew/stocks/pull/1005)); live GCP read on 2026-09-07 with `gcloud` as `claude-web@` (`env -u CLOUDSDK_AUTH_ACCESS_TOKEN gcloud …`; the sandbox's exported token is a placeholder, the SA credential underneath it works) and `scripts/db_query_cr.sh` for table statistics; and the history of the generated docs on GitHub (the clone is shallow). Nothing was carried forward from an older document unverified. The snapshot the new tooling was built and tested against is committed as `tests/fixtures/live_gcp_snapshot_2026-09-07.json`.
...
| Schedulers | 84 (per-hour news and sec-filings entries expanded) | 66, one paused | live-only: `news-sentiment-hourly`, `news-topics-hourly`, `sec-filings-intraday`, `discord-warm-open/-close` (#1004), `phase6-playbook-daily` (#1005); repo-only: the 24 per-hour entries they replaced; `gamma-levels-daily` targets a job `deploy.sh` never creates (#829) but that exists and runs green |
...
5. `docs/product/05` and `README.md` under `docs/product/` state 64 schedulers; live is 66 after the Discord warm window landed (flagged by `verify_docs_against_live.py`, outside this audit's scope).
6. The `--tasks 3` split and 16 Gi sizing of `phase6-playbook` (#1005) and the `pin-images` / `retire-legacy-images` deploy targets (#1004) will appear in ARCHITECTURE.md automatically once those PRs merge and the refresh re-renders the blocks; the prose in §6/§8/§13 already names them as pending.
