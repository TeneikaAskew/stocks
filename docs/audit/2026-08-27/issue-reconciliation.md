...
| [#958](https://github.com/TeneikaAskew/stocks/issues/958) (post-audit) | Closed completed, not moved: the stale `phase1-charts.spec.ts` was deleted on both sides by the split, and the API-connectivity coverage it protected remains in `tests/test_platform_api.py` (63 tests). |
...
| PR-O — Test and CI coverage | #843, #844, #847, #849, #868 (moved to [solyra#28](https://github.com/TeneikaAskew/solyra/issues/28) 2026-09-03) | Shared CI wiring and integration fixtures; avoid mixing product behavior changes into the coverage PR. The frontend Vitest/Playwright half left this repo in the #957 split; #843/#844/#847/#849 remain stocks-side. |
