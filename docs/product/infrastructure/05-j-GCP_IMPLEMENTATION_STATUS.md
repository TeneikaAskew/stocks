...
**Last Updated**: 2026-09-16 (Codex round on #1117, results doc §12: the auditor's HIGH tier counts distinct ET sessions instead of a bar quota that warmup-dropping made unreachable, and judges only the serving model version; the three live magnitude reads take `source = 'inference'` only; the walk-forward per-bar SQL write, failing with SQLSTATE 42804 on every run since #597, binds `ts` as TIMESTAMPTZ; `pred_bucket_proba` added to `/api/magnitude/*` and the expected-move block; `scripts/backfill_model_contracts.py --commit` re-stamped IWM 15m, QQQ 30m and IWM 30m (`c49qf`) with training-label priors taken from the `6hp7l` sibling contracts, and `contract_mismatch` now refuses any other `class_priors_source`; research and base images rebuilt and `magnitude-engine` (gen 178) and `magnitude-inference` (gen 13) updated onto research image `sha256:a0366260…` (Cloud Build `9ac3da56`) and `audit-magnitude-drift` (gen 5) onto base image `sha256:3eecbe92…` (Cloud Build `84eb1a4b`); `magnitude-inference-l9cz5` verified all four serving contracts (IWM 15m under the re-stamped priors) and wrote the 2026-09-15 session, 75/75/75/23 bars; `audit-magnitude-drift-t5p82`: HIGH on IWM 15m `c49qf` (TIGHT on 138/138 over six sessions, the known constant; open decision (a)), SPY 5m `6hp7l` MEDIUM at 97.3% with the two-session reason, IWM/QQQ 5m MEDIUM at 81-83%. Third review pass (`8c8e954e`): the walk-forward's per-bar SQL write is removed (GCS CSV is the evidence), the auditor reads each cell's `LATEST` pointer to pick the version it judges, and the contract check refuses JSON booleans; fourth pass (`ee23d88b`): an empty `LATEST` is a registry error and the backfill's sibling search runs the reader's validation; images rebuilt and the three jobs updated again: `magnitude-engine` (gen 179) and `magnitude-inference` (gen 14) on research image `sha256:7b707cae…`, `audit-magnitude-drift` (gen 6) on base image `sha256:f3b27740…`; `audit-magnitude-drift-7ph2x` under the registry read reports the same four findings as `t5p82`. Fifth pass (`2e49dc83`, `decision_rule` column): research image `sha256:f7d04ebf…` → `magnitude-engine` gen 180, `magnitude-inference` gen 15; the job's startup DDL added the column and `magnitude-inference-2p8xc` (`--lookback-hours=504`) re-scored 14 sessions per serving cell as `lift` rows (the ad-hoc SQL migration was refused by the session's permission classifier and was not needed). Base image rebuilt with the auditor's `decision_rule` filter (Cloud Build `dd6db19d` → `sha256:0198777d…`), `audit-magnitude-drift` gen 7; `audit-magnitude-drift-f8llt` on the 14 migrated sessions: HIGH IWM 15m `c49qf` 137/138, HIGH SPY 5m `6hp7l` 96.7%, HIGH QQQ 5m 93.0%, MEDIUM IWM 5m 75.0%, against a realised TIGHT rate on the same bars of 86% SPY/QQQ and 74% IWM (results doc §12, open decision (e)). Rollback: `magnitude-engine`/`magnitude-inference` to `@sha256:79940375…`, `audit-magnitude-drift` to `@sha256:24f89de6…`; the three re-stamped `CONTRACT.json` payloads are in the session scratchpad.)

**Previous**: 2026-09-15 (magnitude decision-rule deploy, branch `claude/e2e-ci-behavior-1cr1g6` at `9c70f694`: research image rebuilt by Cloud Build `ad36961a` → `trading-system:research@sha256:79940375…` (was `7c3afb98…`, 2026-09-08); `magnitude-engine` (gen 177) and `magnitude-inference` (gen 12) updated onto it; the six serving `CONTRACT.json` files upgraded in place with measured `class_priors` + `decision_lift_min=2.0` by `scripts/backfill_model_contracts.py --commit`; `magnitude-inference-pfr64` verified every served cell's contract and wrote 75 rows/cell under the decision rule; α=0 runs on the new engine: `vpj2r` (phase1, 6/9 clear gates 1-4, no promotion by design) and `6hp7l` (phase0), which **promoted SPY/QQQ/IWM 5m** to `magnitude-engine-6hp7l` (gate 5 100%, gate 6 0.9-1.2×); `magnitude-inference-h4hk7` serves them; `audit-magnitude-drift-d9kkm` flagged SPY 5m HIGH on one calm session; the HIGH tier now needs five sessions of bars per timeframe and the auditor is redeployed on a rebuilt base image (`a4d72306` → `24f89de6…`, gen 4); first run `audit-magnitude-drift-x6hgq` reports no HIGH (results doc §11). First served session predicted-vs-actual recorded in §11 and `docs/research/2026-09-15/`. Rollback: both jobs to `@sha256:7c3afb98…`; LATEST pointers unchanged (recorded). Full suite 5552 passed / 16 pre-existing sandbox failures (yfinance, idna, NYSE calendar), magnitude suites 446 passed)
