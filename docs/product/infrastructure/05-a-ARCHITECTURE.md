...
1. **DDL embedded in the job that owns the table**, applied just-in-time on each run — `gamma_levels_eod` (`gcp/research/p2_build_gamma_levels.py`), `gamma_events` (`gcp/research/p2_outcomes_grid.py`), `magnitude_walk_forward_results` and `magnitude_per_bar_predictions` (`gcp/research/magnitude_engine/mag_walk_forward.py`), and `strat_features_4h` (`gcp/research/strat_engine/strat_data_builder.py`, whose comment notes the other timeframes are not its to apply).
2. **A dedicated DDL file under `gcp/queries/`, applied by hand** — `p7_schema.sql` for the other `strat_features_*` timeframes, `magnitude_engine_schema.sql` for `market_data_indicators*` and `market_data_cross_asset`, `p7_vex_cache.sql` for `daily_vex`. No job runs these. The first two still instruct an operator to dispatch `db-query.yml`, a workflow deleted on 2026-05-30 and replaced by `./scripts/db_query_cr.sh`, so those instructions are stale.
3. **DDL in a retired job, so no active creation path** — the `*_30m_predictions` tables. Their `CREATE TABLE IF NOT EXISTS` still exists, at `gcp/research/_archive/p7a_iwm_30m_pipeline.py:80` and `:373`, but nothing runs it: that file is archived. The table name is built from an f-string (`PREDICTIONS_TABLE = f"{TICKER.lower()}_{TF}_predictions"`), so a literal search for the table name will not find the DDL. They are live residue of a retired pipeline.
