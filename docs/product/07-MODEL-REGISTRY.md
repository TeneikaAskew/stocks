...
| DOC-36 | **FIXED HERE** | The docstring is rewritten (`gcp/weekend_review.py:1-14`) to describe what the module does, naming DOC-36 and [#1137](https://github.com/TeneikaAskew/stocks/issues/1137) so the correction is greppable from the code. *This row previously read "RECORDED — and its issue may close without it", warning that [#1139](https://github.com/TeneikaAskew/stocks/pull/1139) says `Closes #1137` while fixing three other docstrings. That risk is now closed from this side: the file was already being edited for DOC-62, and a one-sentence correction cost nothing next to the chance of the issue closing over it.* #1137 stays open for the docstrings it also covers |
