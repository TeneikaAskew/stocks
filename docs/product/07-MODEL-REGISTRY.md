...
| DOC-24 | **FIXED HERE** in the doc, **FLAGGED** at source | MODEL-EARN-001 now records the duplicate path and quotes the divergence. Routing `refresh_earnings_views.py` through `classify_archetype` changes a production job's output and needs a code PR with a before/after on the affected rows; no issue is filed yet |
...
| DOC-25 | **FIXED HERE** | The duplicate row is deleted, DOC-20's disposition now records that the count was **deleted** rather than corrected a third time, and a new invariant gates the half of the register the old one skipped: no `DOC-nn` may carry two different disposition verdicts. Repeating an id across rows stays legal — DOC-13 genuinely has a registry half and a path half — what is gated is two rows disagreeing about what was *done* |
...
| One `DOC-nn`, one **disposition verdict** | DOC-24 carrying both "FIXED HERE in the doc, FLAGGED at source" and "FLAGGED — needs a code fix". The uniqueness gate above stopped at the Findings table, so half the register was ungated and the suite stayed green |
