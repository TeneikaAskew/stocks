...
| DOC-56 | **FIXED HERE — behaviour change** | Grouped by label from the unrounded score, one rung per row with its observed span, `NaN` counted and surfaced rather than labelled. `tests/gcp/test_weekend_review_score_labels.py`, 11 cases, mutation-tested three ways (restore `int()`, regroup by raw score, drop the `NaN` guard). **Weekly reports before 2026-09-22 used the truncated labels, so their per-rung win rates are not comparable with later ones** |
