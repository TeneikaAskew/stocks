...
| MODEL-MOM-001 | Momentum strategy | Heuristic | long/short eligibility | `lib/strategies/momentum.py` | Production but needs remediation | RETEST | UNVERIFIED · [DOC-10](#documentation-coverage-and-freshness) | [#285](https://github.com/TeneikaAskew/stocks/issues/285) duplicate inline path · [#701](https://github.com/TeneikaAskew/stocks/issues/701) two divergent voters |
| MODEL-MR-001 | Mean reversion strategy | Heuristic | reversion eligibility | `lib/strategies/mean_reversion.py` | Production but needs remediation | RETEST | UNVERIFIED · [DOC-10](#documentation-coverage-and-freshness) | [#249](https://github.com/TeneikaAskew/stocks/issues/249) walk-forward RSI thresholds |
| MODEL-AGREE-001 | Agreement scoring | Heuristic / ensemble | combine strategy evidence into a score | `lib/strategies/agreement.py` | Production but needs remediation | RESTRUCTURE | UNVERIFIED · [DOC-10](#documentation-coverage-and-freshness) | [#905](https://github.com/TeneikaAskew/stocks/issues/905) freeze and prospectively validate expectancy |
...
| MODEL-BRIEF-001 | Brief bias / movement statement | Heuristic | market bias and explanation | `lib/strategies/brief_bias.py`, `lib/movement_statement.py` | Experimental | RETEST | UNVERIFIED · [DOC-10](#documentation-coverage-and-freshness) | **none open** — [#900](https://github.com/TeneikaAskew/stocks/issues/900) closed 2026-09-14; the RETEST rests on no current blocker and needs a stated reason or a status change |
...
| MODEL-EARN-001 | Earnings reaction analytics | Statistical / heuristic | event reaction and strategy lean | `lib/earnings_reactions.py` | Experimental | RETEST | UNVERIFIED · [DOC-10](#documentation-coverage-and-freshness) | [#863](https://github.com/TeneikaAskew/stocks/issues/863) winners posted to Discord at 99 days old |
...
|---|---|---|---|---|---|---|---|---|
| MODEL-MAG-001 | Magnitude prediction | ML | target magnitude class / probability | `gcp/research/magnitude_engine`, `platform/api/routers/magnitude.py` | **Invalidated** — but see below, it is not idle | PAUSE | **CONTRADICTED** · [DOC-06](#documentation-coverage-and-freshness) | Research arc closed **PROJECT VERDICT FAIL by gate 7** ([#575](https://github.com/TeneikaAskew/stocks/pull/575)). Later argmax-collapse incident: predicted TIGHT on 588/588 live bars; promotion gate added in [#810](https://github.com/TeneikaAskew/stocks/pull/810), no-op fixed in [#811](https://github.com/TeneikaAskew/stocks/pull/811). Open: [#874](https://github.com/TeneikaAskew/stocks/issues/874), [#875](https://github.com/TeneikaAskew/stocks/issues/875), [#890](https://github.com/TeneikaAskew/stocks/issues/890) |
...
| MODEL-STYLE-001 | User style mining | ML | learned personal trading pattern | `platform/api/routers/backtest.py` (`/api/style/mine-and-validate`), `user_style_results` | Experimental | RETEST | UNVERIFIED · [DOC-10](#documentation-coverage-and-freshness) | Origin [#707](https://github.com/TeneikaAskew/stocks/pull/707) — walk-forward validated into the playbook seam |
| MODEL-CALIB-001 | Ticker calibration / walk-forward | Statistical | per-ticker thresholds written to production | `lib/walk_forward.py`, `ticker_calibration` | **Invalidated** | RESTRUCTURE | UNVERIFIED · [DOC-04](#documentation-coverage-and-freshness), [DOC-08](#documentation-coverage-and-freshness), [DOC-09](#documentation-coverage-and-freshness), [DOC-10](#documentation-coverage-and-freshness) | [#813](https://github.com/TeneikaAskew/stocks/issues/813) "out-of-sample" calibration is in-sample **and auto-writes production** · [#817](https://github.com/TeneikaAskew/stocks/issues/817) exhaustive in-sample mining, no multiple-testing control · [#886](https://github.com/TeneikaAskew/stocks/issues/886) survivorship bias · [#380](https://github.com/TeneikaAskew/stocks/issues/380) close the loop |
...
> remainder. No other `E-` id is split this way.
...
| DOC-13 | `docs/EXPERIMENT_REGISTRY.md`, `docs/product/README.md` | Dead code paths from a test-tree migration and the solyra frontend split: 3 remaining in the registry, 14 in the product README (`platform/src/routes/*.tsx`, `gcp/freshness_watchdog.py`, `tests/navigation.spec.ts`) — all pre-existing | dead-path | **L** | — |
...
| DOC-06 | **FLAGGED — needs measurement** | Whether recent engine work overturns the gate-7 FAIL is an experiment, not an edit. The verdict stands as recorded; the contradiction is now visible. Resolving it means re-running gate 7 |
| DOC-09 | **FLAGGED — needs live state** | Whether the table is actually stale requires reading live `ticker_calibration`. The swallow (`\|\| echo`) is a real Rule 3.7 defect and is worth its own issue |
| DOC-10, DOC-11 | **FLAGGED — needs your decision** | Writing seven model docs, and deciding which learned models deserve a capability row, is scoped work rather than a correction |
...
| DOC-13 (product README half) | **FLAGGED — pre-existing** | 14 dead paths, all predating this work; pinned in `tests/meta/test_model_registry_consistency.py` so the number can only shrink |
...
| PR | Scope | Overlap with the registry |
|---|---|---|
| [#1008](https://github.com/TeneikaAskew/stocks/pull/1008) | Shared policy ownership, `lib/eastern_time.py` | None. Carries `docs/audits/PR_LANDING_ORDER_AUDIT_2026-09-07.md`, which is **self-marked a superseded snapshot** — do not read it as current evidence |
| [#1060](https://github.com/TeneikaAskew/stocks/pull/1060) | Monthly architecture doc refresh (bot) | Touches `docs/product/infrastructure/05-a,05-c,05-d` and the root README. No model-registry overlap; owns the DOC-12 counts |
| [#1111](https://github.com/TeneikaAskew/stocks/pull/1111) | This work | — |
