...
| Model documentation ownership | `docs/models/` now holds a reference doc per model, and **all but one record at least one operating constant as `UNKNOWN` — only you can supply the rationale**: MODEL-AGREE-001 (`AGREEMENT_BONUS`), MODEL-BRIEF-001 (why 3 and 2), MODEL-CALIB-001 (why a 60-day lookback), MODEL-MOM-001 (the RSI band, `MIN_CORE_CONDITIONS`, three newer thresholds), MODEL-MR-001 (`MIN_CONDITIONS`, the band, `CONSECUTIVE_PERIODS`, the StochRSI bounds), MODEL-STYLE-001 (why ten entries and fourteen warm-up bars), MODEL-SWEEP-001 (that its thresholds generalise at all). MODEL-EARN-001 is the exception. Separately, seven models are absent from the master matrix. *No bare count is published here: an earlier revision said “seven docs, four `UNKNOWN`” and both numbers were wrong — see DOC-28* | [07 DOC-11 and the `UNKNOWN` rationales](07-MODEL-REGISTRY.md#documentation-coverage-and-freshness) | TBD |
