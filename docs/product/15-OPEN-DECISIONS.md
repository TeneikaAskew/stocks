...
| Model documentation ownership | the seven reference docs now exist, but four of them record their thresholds' rationale as `UNKNOWN` — only you can supply it; and seven models are absent from the master matrix | [07 DOC-11 / the four `UNKNOWN` rationales](07-MODEL-REGISTRY.md#documentation-coverage-and-freshness) | TBD |
| MODEL-CALIB-001 row split | the row covers two writers: the scheduled percentile calibrator (`scripts/calibrate_thresholds.py` → `ticker_calibration`) and the on-demand walk-forward sweep (`scripts/run_param_sweep.py` → `exit_config_overrides`). The Invalidated status belongs to the sweep; the calibrator has never been assessed. One row hides that; two rows need a governance decision on ids | [07 DOC-23](07-MODEL-REGISTRY.md#documentation-coverage-and-freshness) · [MODEL-CALIB-001](../models/MODEL-CALIB-001.md) | TBD |
