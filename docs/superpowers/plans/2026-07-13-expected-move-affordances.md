...
- **`pred_bucket` semantics (added 2026-09-14)**: the served bucket is the decision rule P(bucket) ≥ 2× its training prior, not argmax (`mag_pred_train.decide_bucket`). The `size_class` chip should read `pred_bucket`; the magnitude line should read the probabilities. E-35 in `EXPERIMENT_REGISTRY.md`.
