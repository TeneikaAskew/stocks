...
Layout (top→bottom, per spec + mockup): header row (title, TickerCombobox, date picker using `/api/market/dates/{ticker}`, `Mark Entry`, `Add Trade`, CSV/export buttons, `[Examples | My journal]` segmented toggle right-aligned); `row: TradeMarkingChart (flex-1, height clamp like Charts) + trade rail (w-[340px]: per-date TradeRailCards + session-stats mini card)`; KPI tiles + practice toggle + notes; equity curve card (placeholder preserved); full trade table + NEW columns Stop / TPs / R:R (riskReward helper; "—" honesty) + existing actions (disabled with tooltip in Examples view).
