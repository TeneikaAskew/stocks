...
| [`rerun-transient-doc-refresh.yml`](rerun-transient-doc-refresh.yml) | `workflow_run` on `Monthly architecture doc refresh` completing | Re-runs the failed jobs **once** when that workflow died on a Vertex transport stall (`UND_ERR_BODY_TIMEOUT` and friends), which cost run 20 a whole 13-minute run on 2026-09-07. It classifies on the CLI's line-anchored fatal record plus a transport cause in the last 40 lines of the failed step, so a real repo failure (runs 15, 16, 21) stays red and a signature the model echoed into its own stdout does not count. Bounded by `run_attempt == 1`, so it cannot loop. Re-running the job rather than retrying inside it means the attempt gets a fresh checkout, which is why none of the workspace-trust guards an in-job retry needs apply here. |
