...
Every workflow with a main job ends in a `handle-failure` job wired to `handle-workflow-failure.yml` (CLAUDE.md "Automated Workflow Failure Handling"); a workflow without one fails silently, which is how the 2026-06 and 2026-07 doc refreshes went unnoticed for two months.
