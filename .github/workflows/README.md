...
| [`rerun-transient-doc-refresh.yml`](rerun-transient-doc-refresh.yml) | `workflow_run` on `Monthly architecture doc refresh` completing | Re-runs the failed jobs **once** when that workflow died on a Vertex transport stall (`UND_ERR_BODY_TIMEOUT` and friends), which cost run 20 a whole 13-minute run on 2026-09-07. It classifies on the CLI's line-anchored fatal record plus a transport cause inside the failed STEP's own section of the log, cut by the step's `started_at`/`completed_at` from the jobs API against the timestamp the runner prefixes onto every line. An earlier version took the last 40 lines of the whole job, which run 20 disproves: its failed job's last 42 lines are the `if: failure()` artifact upload and post-job cleanup, with no Gemini record among them. So a real repo failure (runs 15, 16, 21) stays red and a signature the model echoed into its own stdout does not count. Bounded by `run_attempt == 1`, so it cannot loop. It reads **raw per-job logs** rather than `gh run view --log-failed`, whose `<job>\t<step>\t<timestamp>` column prefixes would defeat the anchored match and make the whole workflow a silent no-op. A second job closes the obsolete `fix/workflow-…` draft PR once a re-run succeeds, and a `handle-failure` job means a failure of the recovery itself is not silent. Re-running the job rather than retrying inside it means the attempt gets a fresh checkout, which is why none of the workspace-trust guards an in-job retry needs apply here. |
