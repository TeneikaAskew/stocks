...
1. Read `previous/docs/product/infrastructure/05-a-ARCHITECTURE.md` and the current `docs/product/infrastructure/05-a-ARCHITECTURE.md`. They differ inside the marker blocks, and in exactly **four** lines of prose outside them, all four rendered into the current file by the workflow before you run:
   - the **runtime-relation count**, from live minus declared. It is already the number in the **Live fleet counts** block above.
   ...
   **In all four the current file is right and `previous/` is stale by construction.** Never carry any of them back from the previous version, and do not "restore" a difference you find in these four lines — it is the workflow's edit, not drift. Everything else outside the blocks should match, and a difference there is yours to reconcile.
