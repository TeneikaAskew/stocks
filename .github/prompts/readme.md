...
2. Badges: **already rendered before you run — do not touch them.** The workflow writes the five badge lines from the inventory (verified date, jobs live/declared, schedulers live, schema relations declared/live, and the refresh-workflow status badge). Leave the block byte-for-byte as you find it. Run 32 rewrote it and pointed the workflow badge at `refresh-documentation.yml`, a file that does not exist, which failed the run on a dead link; you no longer have that job.
3. **Documentation map**: one row per document, "Read this | When you want". **The "When you want" text on each existing row is hand-written and stays byte-for-byte** — it is specific on purpose ("the whole system: every job, scheduler, service, table, route, deploy path, data flow, failure path, live-vs-repo reconciliation" is more useful than "understand the system's components"). Your only edits to this table are adding a row for a document that now exists and removing one whose file is gone. Run 32 reworded every row into generic descriptions and got two of them factually wrong. Must link `docs/product/infrastructure/05-a-ARCHITECTURE.md`, `docs/product/infrastructure/05-c-DATA_DEPENDENCIES.md`, `docs/product/infrastructure/05-d-COST_ANALYSIS.md`, `RUNBOOK.md`, `docs/product/infrastructure/05-b-ERD.md`, `docs/product/infrastructure/05-f-PIPELINE.md`, `docs/product/infrastructure/05-g-DATA_PIPELINE.md`, `docs/product/infrastructure/05-e-API.md`, `docs/product/infrastructure/05-i-GCP_IMPLEMENTATION_GUIDE.md`, `docs/product/README.md`, `docs/audits/`, `gcp/cloudbuild/README.md`, `SETUP.md`, `CLAUDE.md`, and the solyra repo. Add a row for any new top-level or `docs/` reference document; drop rows whose file no longer exists.
