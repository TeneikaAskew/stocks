...
- The current `docs/product/infrastructure/05-c-DATA_DEPENDENCIES.md`. Read only the prose you may edit (named below) with `read_file` `offset`/`limit`; the rendered blocks between markers are ~120 KB of the file and are not yours to read or change. Read each prose region **whole** before you touch it — widen the window until you have the complete paragraph, including its last sentence. A `replace` must carry the entire new text of the span it replaces; if you have only seen part of a paragraph you cannot rewrite it, and abbreviating the part you did not read deletes it. The previous version under `previous/` differs from the current one inside those blocks and in the one prose line carrying the **runtime-relation count**, which the workflow renders into the current file before you run; the current file's number is correct and the previous one is stale by construction. There is nothing else to compare.
...
- **A horizontal rule (`---`) stays alone on its line, and a date that is part of a path is never an as-of date to update.** Run 30 did both to 05-a: it welded the closing rule onto the provenance line and bumped a dated audit filename in a link, inventing a file that does not exist. Gates now fail the run on each. Retargeting a link because the code moved is a different thing and is still expected; what is forbidden is editing a date inside a path.
