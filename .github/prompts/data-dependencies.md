...
- **A horizontal rule (`---`) stays alone on its line, and a date inside a filename or link is never an as-of date to update.** Run 30 did both to 05-a: it welded the closing rule onto the provenance line and bumped a dated audit filename in a link, inventing a file that does not exist. Gates now fail the run on each.
